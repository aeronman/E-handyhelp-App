    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        profiles = jsonResponse.map((profile) {
          ...
          if (imageUrl is List) {
            imageUrl = imageUrl.isNotEmpty ? imageUrl[0] : ''; // Get the first image if it's a list
              ...
              const SizedBox(height: 40),
                ...
                if (profiles.isEmpty) // Check if profiles is empty
                  ...
                  ListView.builder(
                    ...
                    itemCount: profiles.length, // Use profiles.length for dynamic data
                    itemBuilder: (context, index) {
                      // Logging the values to check for null
                      String name = profiles[index]['name'] ?? 'Unknown';
                      if (profiles[index]['name'] == null) print('Null detected in "name" at index $index');

                      String address = profiles[index]['address'] ?? 'No address';
                      if (profiles[index]['address'] == null) print('Null detected in "address" at index $index');

                      String contact = profiles[index]['contact'] ?? 'No contact';
                      if (profiles[index]['contact'] == null) print('Null detected in "contact" at index $index');

                      String imageUrl = profiles[index]['imageUrl'] ?? 'assets/default_image.png'; // Ensure a fallback image
                      if (profiles[index]['imageUrl'] == null) print('Null detected in "imageUrl" at index $index');

                      String serviceDetails = profiles[index]['serviceDetails'] ?? 'No details';
                      if (profiles[index]['serviceDetails'] == null) print('Null detected in "serviceDetails" at index $index');

                      String dateOfService = profiles[index]['dateOfService'] ?? 'No date';
                      if (profiles[index]['dateOfService'] == null) print('Null detected in "dateOfService" at index $index');

                      List<String> serviceImages = profiles[index]['serviceImages'] != null &&
                              profiles[index]['serviceImages'] is List<String>
                          ? profiles[index]['serviceImages']
                          : [];
                      if (profiles[index]['serviceImages'] == null) print('Null detected in "serviceImages" at index $index');
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => ProfileDetailPage(
                                    ...
                                    serviceImages: serviceImages, // Pass the list of images
                                  ),
                                ),
                              ).then((value) {
                                if (value == true) {
          ...
          body: PhotoViewGallery.builder(
            ...
            builder: (context, index) {
              return PhotoViewGalleryPageOptions(
                imageProvider: MemoryImage(base64Decode(widget.serviceImages[index])),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 16),
                  Text('Date of Service:', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  ...
                  SizedBox(height: 24),
                  Text('Service Images:', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  SizedBox(height: 8),
                  ...
                  widget.serviceImages.isNotEmpty
                      ? Wrap(
                          spacing: 8.0,
                          runSpacing: 8.0,
                          children: widget.serviceImages.asMap().entries.map((entry) {
                            int index = entry.key;
                            String imageBase64 = entry.value;
                            return GestureDetector(
                              onTap: () {
                                _showImageGallery(context, index); // Show zoomable image on tap
                              },
                              child: Container(
                                width: 100,
                                height: 100,
                    ...
                    children: [
                      ElevatedButton(
                        onPressed: _isLoading ? null : () {
                          acceptBooking(context); // Call accept booking
                        },
                      ...
                      ElevatedButton(
                        onPressed: _isLoading ? null : () {
                          declineBooking(context); // Call decline booking
                        },
                        style: ElevatedButton.styleFrom(
