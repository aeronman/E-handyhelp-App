...
class _NotificationPageState extends State<NotificationPage> {
  ...
  Future<void> _fetchNotifications() async {
    final response = await http.get(Uri.parse('https://8d15a120-59ff-4395-9b44-876920f1d072-00-9xsue14fhvuy.worf.replit.dev/api/handynotifications/$_id'));
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        notifications = data.map((item) => NotificationItem(
          ...
          date: DateTime.parse(item['date']), // Assuming date is in a parseable string format
        )).toList();
      });
    } else {
