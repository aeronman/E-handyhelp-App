...
Future<void> _saveUserData(Map<String, dynamic> responseData) async {
  ...
  if (responseData.containsKey('user')) {
    ...
    await prefs.setString('token', responseData['token'] ?? ''); // Handle null token
    await prefs.setString('_id', user['_id'] ?? '');  // Handle null _id
    await prefs.setString('username', user['username'] ?? '');  // Handle null username
    await prefs.setString('fname', user['fname'] ?? '');  // Handle null fname
    await prefs.setString('lname', user['lname'] ?? '');  // Handle null lname
    await prefs.setString('contact', user['contact'] ?? '');  // Handle null contact
    await prefs.setString('dateOfBirth', user['dateOfBirth'] ?? '');  // Handle null dateOfBirth
    await prefs.setString('accounts_status', user['accounts_status'] ?? '');  // Handle null status
    
    // Convert images list to a JSON string
    if (user['images'] is List) {
      await prefs.setString('images', jsonEncode(user['images']));
    } else {
      await prefs.setString('images', '[]'); // Default to an empty list if not a list
    }
  } else {
