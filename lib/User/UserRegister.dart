 ...
 Future<void> _registerUser() async {
  ...
  if (_images.isEmpty) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Please upload a photo of your Barangay ID.')),
    );
  ...
  if (!_dataPrivacyConsent) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('You must agree to the Data Privacy Act.')),
    );
    print('User registration failed: Data privacy consent not agreed'); // Log message
    return;
  }

  final url = Uri.parse('https://8d15a120-59ff-4395-9b44-876920f1d072-00-9xsue14fhvuy.worf.replit.dev/register');
  ...
  try {
    ...
    if (response.statusCode == 201) {
      ...
      print('Registration successful for username: $_username'); // Log success message
      ...
      await prefs.setString('dateOfBirth', _dateOfBirth?.toLocal().toString().split(' ')[0] ?? '');
      ...
      // Registration failed
      print('Registration failed with status code: ${response.statusCode}'); // Log failure
      ScaffoldMessenger.of(context).showSnackBar(
