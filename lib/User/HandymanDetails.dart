...
class _HandymanDetailsPageState extends State<HandymanDetailsPage> {
  ...
  Future<void> _fetchFeedbacks() async {
    final url =
        'https://8d15a120-59ff-4395-9b44-876920f1d072-00-9xsue14fhvuy.worf.replit.dev/feedbacks?handymanId=${widget.handymanId}';
    ...
    if (response.statusCode == 200) {
      try {
        ...
        averageRating = double.tryParse(decodedResponse['averageRating']) ?? 0.0;
      ...
      print('Failed to fetch feedbacks: ${response.statusCode} - ${response.body}');
    }
  }
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          SizedBox(width: 4),
                          Text(feedback['rating'].toString(), style: TextStyle(fontSize: 16)),
                        ],
                      ),
                    ],
                  ),
  ...
  Widget _buildHandymanInfo() {
    return Column(
      ...
      children: [
        ...
        SizedBox(height: 8),
        Text('Handyman Type: ${widget.handymanType ?? 'N/A'}', style: TextStyle(fontSize: 16)),
        SizedBox(height: 8),
        Text('Contact: ${widget.contact ?? 'N/A'}', style: TextStyle(fontSize: 16)),
        SizedBox(height: 8),
        Text('Address: ${widget.address ?? 'N/A'}', style: TextStyle(fontSize: 16)),
      ],
    );
  }
